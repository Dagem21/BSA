...
const sectors = [
    ...
    "Hotels and Tourism", "Transport and Communication", 
    "Financial Institutions", "Real Estate", "Personal Loans", 
    "Mining and Quarrying", "Health and Education", "Others"
];

const loanCategories = ["Short Term", "Medium Term", "Long Term", "Overdraft", "Pre-shipment", "Post-shipment"];
...
for (let i = 0; i < 45; i++) {
    ...
    const outstanding = (Math.random() * 1000 + 50).toFixed(2);
    const accounts = Math.floor(Math.random() * 500) + 10;
    const minRate = (Math.random() * 5 + 7).toFixed(2);
    const maxRate = (Math.random() * 5 + 13).toFixed(2);
    const weightedRate = (Math.random() * 3 + 10).toFixed(2);
    const weightedAvgRate = (Math.random() * 2 + 11).toFixed(2);
...
const outputPath = "C:\\Users\\mulukenwalle\\Desktop\\national_bank\\BSA\\reports\\json\\MWAL001_sample_45rows.json";
fs.writeFileSync(outputPath, JSON.stringify(reportData, null, 4));
console.log(`Successfully generated 45 rows of sample data at ${outputPath}`);
