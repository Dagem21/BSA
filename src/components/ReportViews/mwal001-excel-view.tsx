...
interface DynamicArea {
    Area: number;
    _areaName: string;
    DynamicItems: DynamicItem[]; // MWAL001 uses flat arrays inside DynamicItems
}

interface MWAL001JsonData {
    ReturnKey?: string;
    InstCode?: string;
    FinYear?: number;
    StartDate?: string;
...
interface MWAL001ExcelViewProps {
    initialData?: MWAL001JsonData;
...
export function MWAL001ExcelView({ initialData, activeFileName }: MWAL001ExcelViewProps) {
    ...
    const headers = [
        ...
        { code: "1.7", label: "Weighted Rate", colLetter: "G" },
        { code: "1.8", label: "Weighted Avg Rate", colLetter: "H" }
    ];
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "IFBLCMWAL001"}
                            </span>
                        </div>
