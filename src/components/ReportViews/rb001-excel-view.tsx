...
interface RB001ExcelViewProps {
    initialData?: RB001JsonData;
...
export function RB001ExcelView({ initialData, activeFileName }: RB001ExcelViewProps) {
    ...
    const [selectedCell, setSelectedCell] = useState<{
        ...
        cellRef: "C14",
        code: "166_00001",
        rowDesc: "Reserve Base (1.1+1.2+1.3)",
        colSuffix: "Day 1",
        value: "0"
    });
    ...
    const getItemValue = (itemCodeNum: number): string => {
        const codeStr = `166_${itemCodeNum.toString().padStart(5, "0")}`;
    ...
    let codeCounter = 1;
    const gridRows = RB001_ROW_DESCRIPTIONS.map((desc, rowIndex) => {
        const rowExcelNum = 14 + rowIndex;
        const rowItems = RB001_COL_SUFFIXES.map((colSuffix, colIndex) => {
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "Reserve BaseRB001"}
                            </span>
                        </div>
