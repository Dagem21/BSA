...
import {
    ZS001_ROW_DESCRIPTIONS,
    ZS001_COL_SUFFIXES
} from "@/utils/services/ZS001/jsonFormat";
...
export function ZS001ExcelView({
    initialData,
    activeFileName
}: ZS001ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<ZS001JsonData | undefined>(
        initialData
    );
        ...
        if (!isLoading && data) {
            ...
            toast.error(
                errors.details?.response?.data?.error ||
                    "Failed to fetch ZS001 JSON view"
            );
        }
                        ...
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            <thead>
                                <tr className="border-b border-stroke bg-gray-100 text-center font-mono text-[11px] font-semibold text-gray-600 dark:border-dark-3 dark:bg-dark-2 dark:text-gray-300">
                                    ...
                                    <th className="min-w-[280px] border-r border-stroke py-1.5 pl-3 text-left dark:border-dark-3">
                                        C
                                    </th>
                ...
                <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
                    <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-lg font-bold text-dark dark:text-white">
                            Raw JSON Payload (
                            {reportData?.ReturnItemsList?.length || 72} items)
                        </h2>
                        <button
                            onClick={() => {
