...
import { SRRYY001_ROW_DESCRIPTIONS, SRRYY001_COL_SUFFIXES } from "@/utils/services/SRRYY001/jsonFormat";
...
interface SRRYY001ExcelViewProps {
    initialData?: SRRYY001JsonData;
    activeFileName?: string;
}

export function SRRYY001ExcelView({ initialData, activeFileName }: SRRYY001ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<SRRYY001JsonData | undefined>(initialData);
    ...
    const [selectedCell, setSelectedCell] = useState<{
        ...
        cellRef: "C13",
        code: "165_00001",
        rowDesc: "Net Average Reserve Base (Previous Calendar Month)",
        colSuffix: "Day 1",
        value: "0"
    });
    ...
    const fetchJsonData = async (fileName?: string) => {
        try {
            setLoading(true);
            const query = fileName ? `?filename=${encodeURIComponent(fileName)}` : "?type=SRR";
            ...
            console.error("Failed to fetch SRRYY001 JSON view:", e);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getItemValue = (itemCodeNum: number): string => {
        const codeStr = `165_${itemCodeNum.toString().padStart(5, "0")}`;
    ...
    const netAvgReserveBase = getItemValue(36);  // 165_00036
    const dailyReserveReq = getItemValue(72);     // 165_00072
    const nbeAccountBal = getItemValue(108);      // 165_00108
    const excessDeficiency = getItemValue(144);   // 165_00144

    let codeCounter = 1;
    const gridRows = SRRYY001_ROW_DESCRIPTIONS.map((desc, rowIndex) => {
        const rowExcelNum = 13 + rowIndex;
        const rowItems = SRRYY001_COL_SUFFIXES.map((colSuffix, colIndex) => {
            const itemCodeNum = codeCounter++;
            const codeStr = `165_${itemCodeNum.toString().padStart(5, "0")}`;
            const val = getItemValue(itemCodeNum);

            // Generate Excel col letters (Col C is 3)
            let colLetter = "";
            let c = colIndex + 3;
            while (c > 0) {
                let mod = (c - 1) % 26;
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "SRRYY001"}
                            </span>
                        </div>
                        <h1 className="mt-2 text-2xl font-bold text-dark dark:text-white">
                            Statutory Reserve Requirement Report (SRRYY001)
                        </h1>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-4 dark:border-purple-500/30">
                        <span className="text-xs font-semibold uppercase tracking-wider text-purple-600 dark:text-purple-400">
                            NBE Settlement Account
                        </span>
                        <div className="mt-1 text-xl font-bold text-dark dark:text-white">
                            {formatNum(nbeAccountBal)}
                        </div>
                    ...
                    <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 dark:border-amber-500/30">
                        ...
                        <div className="mt-1 text-xl font-bold text-dark dark:text-white">
                            {formatNum(excessDeficiency)}
                        </div>
                        <span className="text-xs text-gray-500">Monthly Avg (Row 4)</span>
                    </div>
                </div>
            </div>
                        ...
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            <thead>
                                ...
                                <tr className="border-b border-stroke bg-gray-200 font-semibold text-dark dark:border-dark-3 dark:bg-dark-3 dark:text-white">
                                    ...
                                    {SRRYY001_COL_SUFFIXES.map((suffix, i) => (
                            ...
                            <tbody>
                                {filteredRows.map((row) => (
                                    <tr
                                        key={row.rowIndex}
                                        className="border-b border-stroke hover:bg-gray-50 transition dark:border-dark-3 dark:hover:bg-dark-2/50"
                                    >
                ...
                <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
                    <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-lg font-bold text-dark dark:text-white">
                            Raw JSON Payload ({reportData?.ReturnItemsList?.length || 180} items)
                        </h2>
                        <button
                            onClick={() => {
