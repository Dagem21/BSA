...
interface MK001ExcelViewProps {
    initialData?: MK001JsonData;
    activeFileName?: string;
}

const ITEMS_CONFIG = [
    { code: "107_00001", rowNum: 15, desc: "Total assets", indent: 0, isTotal: true },
    { code: "107_00002", rowNum: 16, desc: "Total loans and bonds", indent: 0, isTotal: false },
    { code: "107_00003", rowNum: 17, desc: "Of which bonds", indent: 1, isTotal: false },
    { code: "107_00008", rowNum: 18, desc: "Total deposits", indent: 0, isTotal: true },
    { code: "107_00004", rowNum: 19, desc: "Demand/Current deposits", indent: 1, isTotal: false },
    { code: "107_00005", rowNum: 20, desc: "Saving deposits", indent: 1, isTotal: false },
    { code: "107_00006", rowNum: 21, desc: "Time/Fixed deposits", indent: 1, isTotal: false },
    { code: "107_00007", rowNum: 22, desc: "Total capital & reserves", indent: 0, isTotal: true }
];

export function MK001ExcelView({ initialData, activeFileName }: MK001ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<MK001JsonData | undefined>(initialData);
    ...
    const fetchJsonData = async (fileName?: string) => {
        try {
            setLoading(true);
            const query = fileName ? `?filename=${encodeURIComponent(fileName)}` : "?type=MK001";
            ...
            console.error("Failed to fetch MK001 JSON view:", e);
        } finally {
            setLoading(false);
        }
    };
    ...
    const totalAssets = getItemValue("107_00001");
    const totalDeposits = getItemValue("107_00008");
    const totalLoansBonds = getItemValue("107_00002");
    const totalCapital = getItemValue("107_00007");

    const filteredItems = ITEMS_CONFIG.filter((i) =>
            ...
            <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "Key Balance SheetMK001"}
                            </span>
                        </div>
                        <h1 className="mt-2 text-2xl font-bold text-dark dark:text-white">
                            Monthly Key Balance Sheet (MK001)
                        </h1>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4 dark:border-emerald-500/30">
                        ...
                        <span className="text-xs text-gray-500">Code: 107_00001 (In Millions Birr)</span>
                    </div>
                    ...
                    <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-4 dark:border-blue-500/30">
                        ...
                        <div className="mt-1 text-xl font-bold text-dark dark:text-white">
                            {formatNum(totalLoansBonds)}
                        </div>
                        <span className="text-xs text-gray-500">Code: 107_00002</span>
                    </div>
                    ...
                    <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-4 dark:border-purple-500/30">
                        ...
                        <div className="mt-1 text-xl font-bold text-dark dark:text-white">
                            {formatNum(totalDeposits)}
                        </div>
                        <span className="text-xs text-gray-500">Code: 107_00008</span>
                    </div>
                    ...
                    <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 dark:border-amber-500/30">
                        ...
                        <span className="text-xs text-gray-500">Code: 107_00007</span>
                    </div>
                </div>
            </div>
                        ...
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            <thead>
                                <tr className="border-b border-stroke bg-gray-100 text-center font-mono text-[11px] font-semibold text-gray-600 dark:border-dark-3 dark:bg-dark-2 dark:text-gray-300">
                                    ...
                                    <th className="min-w-[300px] border-r border-stroke py-1.5 dark:border-dark-3">B</th>
                                </tr>
                                <tr className="border-b border-stroke bg-gray-200 font-semibold text-dark dark:border-dark-3 dark:bg-dark-3 dark:text-white">
                                    ...
                                    <th className="border-r border-stroke p-2 dark:border-dark-3">Particulars (In Millions of Birr)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredItems.map((item) => {
                                    const val = getItemValue(item.code);
                                    const isSelected = selectedCell?.cellRef === `B${item.rowNum}`;
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={cn(
                                                ...
                                                item.isTotal && "bg-gray-100 font-bold dark:bg-dark-2 text-dark dark:text-white",
                                                !item.isTotal && "hover:bg-gray-50 dark:hover:bg-dark-2/50"
                                            )}
                                        >
                                            ...
                                            <td className="border-r border-stroke p-2 dark:border-dark-3">
                                                <div className="flex items-center justify-between">
                                                    <span style={{ paddingLeft: `${item.indent * 20}px` }}>
                                                        {item.desc}
                                                    </span>
                                                    <span
                                                        onClick={() =>
                                                            setSelectedCell({
                                                                cellRef: `B${item.rowNum}`,
                                                                code: item.code,
                                                                colName: item.desc,
                                                                value: val
                                                            })
                                                        ...
                                                        className={cn(
                                                            "font-mono cursor-pointer px-3 py-1 rounded transition",
                                                            isSelected
                                                                ? "ring-2 ring-emerald-500 bg-emerald-500/20 font-bold text-emerald-900 dark:text-emerald-200"
                ...
                <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
                    <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-lg font-bold text-dark dark:text-white">
                            Raw JSON Payload ({reportData?.ReturnItemsList?.length || 8} items)
                        </h2>
                        <button
                            onClick={() => {
