...
interface WAADIR001ExcelViewProps {
    initialData?: WAADIR001JsonData;
    activeFileName?: string;
}

export function WAADIR001ExcelView({
    initialData,
    activeFileName
}: WAADIR001ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<WAADIR001JsonData | undefined>(
        initialData
    );
        ...
        if (!isLoading && data) {
            ...
            toast.error(
                errors.details?.response?.data?.error ||
                    "Failed to fetch WAADIR001 JSON view"
            );
        }
    ...
    dynamicRows.forEach((row) => {
        ...
        if (!isNaN(num)) {
            if (row.Code.endsWith("3")) totalDepositAmount += num;
            if (row.Code.endsWith("4")) totalAccounts += num;
            if (row.Code.endsWith(".7")) {
                totalRateSum += num;
                rateCount++;
            }
        }
    ...
    const filteredRows = dynamicRows.filter((row) => {
        ...
        return (
            ...
            row._description.toLowerCase().includes(q)
        );
    });

    const headers = [
        { code: "1", label: "Deposit Type", colLetter: "A" },
        { code: "2", label: "Deposit Category", colLetter: "B" },
        {
            code: "3",
        ...
        {
            code: "4",
            label: "No. of Deposit Accounts by Category",
            colLetter: "D"
        },
        {
            code: "5",
            label: "Minimum Rate by Deposit category",
            colLetter: "E"
        },
        {
            code: "6",
            label: "Maximum Rate by Deposit category ",
            colLetter: "F"
        },
        {
            code: "7",
            label: "Weighted Average Rate by Deposit category",
            colLetter: "G"
        },
        {
            code: "8",
            label: "Weighted Average Rate by Deposit Type",
            colLetter: "H"
        }
    ];
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "WAADIR001"}
                            </span>
                        </div>
                        <h1 className="mt-2 text-2xl font-bold text-dark dark:text-white">
                            Monthly Weighted Average Deposit Interest Rates
                            <br />
                            (Conventional Banks)
                        </h1>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4 dark:border-emerald-500/30">
                        ...
                        <div className="mt-1 text-xl font-bold text-dark dark:text-white">
                            {
                                dynamicRows.filter((item) =>
                                    item.Code.includes(".8")
                                ).length
                            }{" "}
                            Categories
                        </div>
                    ...
                    <div className="overflow-x-auto rounded-lg border border-stroke dark:border-dark-3">
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            <thead>
                                <tr className="border-b border-stroke bg-gray-200 font-semibold text-dark dark:border-dark-3 dark:bg-dark-3 dark:text-white">
                                    ...
                                    {headers.map((h) => (
                                        <th
                                            key={h.code}
                                            className="min-w-[220px] border-r border-stroke p-2 dark:border-dark-3"
                                        >
                                            {h.label}
                                        </th>
                                    ))}
                            ...
                            <tbody>
                                {Array.from(
                                    {
                                        length: filteredRows.filter((item) =>
                                            item.Code.includes(".8")
                                        ).length
                                    },
                                    (_, index) => index
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {headers.map((h) => {
                                                const hCode = `${rIdx + 1}.${h.code}`;
                                                const item = filteredRows.find(
                                                    (i) => i.Code === hCode
                                                );
                                                const val = item
                                                    ? item.Value
