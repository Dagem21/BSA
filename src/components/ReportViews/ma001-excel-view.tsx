...
import {
    MA001_ROW_DESCRIPTIONS,
    MA001_COL_SUFFIXES
} from "@/utils/services/MA001/jsonFormat";
...
export function MA001ExcelView({
    initialData,
    activeFileName
}: MA001ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<MA001JsonData | undefined>(
        initialData
    );
        ...
        if (!isLoading && data) {
            ...
            toast.error(
                errors.details?.response?.data?.error ||
                    "Failed to fetch MA001 JSON view"
            );
        }
            ...
            <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 dark:border-amber-500/30">
                        ...
                        <span className="text-xs text-gray-500">
                            Row 2.4 Total (Assets - Liabilities)
                        </span>
                    </div>
                        ...
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            ...
                            <tbody>
                                {filteredRows.map((row) => {
                                    ...
                                    const isLiabilitiesHeader =
                                        row.code === "2";
                                    const isSectionHeader =
                                        isAssetsHeader || isLiabilitiesHeader;
                                    const isTotalRow =
                                        row.code === "1.3" ||
                                        row.code === "2.3";
                                    const isMismatchRow =
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={cn(
                                                ...
                                                isLiabilitiesHeader &&
                                                    "bg-blue-500/10 font-bold text-blue-900 dark:bg-blue-500/20 dark:text-blue-300",
                                                isTotalRow &&
