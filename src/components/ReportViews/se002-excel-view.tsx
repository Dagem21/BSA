...
const SE002_COL_ORG_SUFFIXES: string[] = [
    "Public Enterprise",
    "Private & Coop.",
    "Regional Gov't",
...
const SE002_COL_SUFFIXES: string[] = [
    "Amount",
    "# of Borrowers",
    "# of Accounts",
...
const SE002_ROW_DESCRIPTIONS: string[] = [
    "Addis Ababa",
    "Term loan",
    "Overdraft",
    "Merch. Loan*",
...
interface SE002ExcelViewProps {
    initialData?: SE002JsonData;
    activeFileName?: string;
}

export function SE002ExcelView({
    initialData,
    activeFileName
}: SE002ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<SE002JsonData | undefined>(
        initialData
    );
        ...
        if (!isLoading && data) {
            ...
            toast.error(
                errors.details?.response?.data?.error ||
                    "Failed to fetch SE002 JSON view"
            );
        }
    ...
    const getItemValue = (itemCodeNum: number): string => {
        const codeStr = `SE002_${itemCodeNum.toString().padStart(5, "0")}`;
    ...
    let codeCounter = 39092;
    const gridRows = SE002_ROW_DESCRIPTIONS.map((desc, rowIndex) => {
        const code = ROW_CODES[rowIndex] || "";
        const rowExcelNum = 17 + rowIndex;
        const rowItems = SE002_COL_SUFFIXES.map((colSuffix, colIndex) => {
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "LOAN_RAN&REG_RA002"}
                            </span>
                        </div>
                        ...
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            <thead>
                                ...
                                <tr className="border-b border-stroke bg-gray-200 font-semibold text-dark dark:border-dark-3 dark:bg-dark-3 dark:text-white">
                                    ...
                                    {SE002_COL_SUFFIXES.map((COL, index) => (
                                        <th
                                            className="border-r border-stroke p-2 text-center text-gray-500 dark:border-dark-3"
                                            key={index}
                                        >
