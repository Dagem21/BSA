...
const RWW002_COL_SUFFIXES: string[] = [
    "<= 100,000_Amount",
...
const RWW002_ROW_DESCRIPTIONS: string[] = [
    "Addis Ababa",
    "Term loan",
    "Overdraft",
    "Merch. loan",
...
interface RWW002ExcelViewProps {
    initialData?: RWW002JsonData;
    activeFileName?: string;
}

export function RWW002ExcelView({
    initialData,
    activeFileName
}: RWW002ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<RWW002JsonData | undefined>(
        initialData
    );
        ...
        if (!isLoading && data) {
            ...
            toast.error(
                errors.details?.response?.data?.error ||
                    "Failed to fetch RWW002 JSON view"
            );
        }
    ...
    const getItemValue = (itemCodeNum: number): string => {
        const codeStr = `WW002_${itemCodeNum.toString().padStart(5, "0")}`;
    ...
    const gridRows = RWW002_ROW_DESCRIPTIONS.map((desc, rowIndex) => {
        const code = ROW_CODES[rowIndex] || "";
        const rowExcelNum = 17 + rowIndex;
        const rowItems = RWW002_COL_SUFFIXES.map((colSuffix, colIndex) => {
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "BD_L&A_RWW002"}
                            </span>
                        </div>
