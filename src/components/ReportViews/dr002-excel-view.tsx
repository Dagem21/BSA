...
import { DR002_REGIONS, DR002_SUB_ROWS, DR002_METRIC_DESCRIPTIONS } from "@/utils/services/DR002/jsonFormat";
...
interface DR002ExcelViewProps {
    initialData?: DR002JsonData;
    activeFileName?: string;
}

export function DR002ExcelView({
    initialData,
    activeFileName
}: DR002ExcelViewProps) {
    ...
    const [reportData, setReportData] = useState<DR002JsonData | undefined>(
        initialData
    );
        ...
        if (!isLoading && data) {
            ...
            toast.error(
                errors.details?.response?.data?.error ||
                    "Failed to fetch DR002 JSON view"
            );
        }
    ...
    const handleDownloadExcel = async () => {
        try {
            ...
            if (reportData) {
                ...
                const a = document.createElement("a");
                a.href = url;
                a.download = `${reportData.ReturnKey || "DR002"}_export.xlsx`;
                document.body.appendChild(a);
                a.click();
                a.remove();
                window.URL.revokeObjectURL(url);
    ...
    const addisTotalAmt = getItemValue("DR002_34691");
    const addisTotalDep = getItemValue("DR002_34692");
    const addisTotalAcc = getItemValue("DR002_34693");

    let totalDepositAmtAll = 0;
    let codeStart = 34682;

    DR002_REGIONS.forEach((_, regIdx) => {
        const headTotalAmtCode = `DR002_${codeStart + regIdx * 72 + 9}`;
    ...
    DR002_REGIONS.forEach((regName) => {
        DR002_SUB_ROWS.forEach((subRow) => {
            ...
            DR002_METRIC_DESCRIPTIONS.forEach((metricDesc, colIdx) => {
                ...
                const isInteger = colIdx % 3 !== 0; // Col 0, 3, 6, 9 are Amount (float), others are integer counts
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded bg-blue-500/10 px-2.5 py-1 text-xs font-semibold text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
                                {reportData?.ReturnKey || "DEP_RAN&REG_DR002"}
                            </span>
                        </div>
                        <h1 className="mt-2 text-2xl font-bold text-dark dark:text-white">
                            Deposit by Range and Region (DR002)
                        </h1>
                        ...
                        <table className="w-full border-collapse text-left text-xs font-normal">
                            <thead>
                                <tr className="border-b border-stroke bg-gray-100 text-center font-mono text-[11px] font-semibold text-gray-600 dark:border-dark-3 dark:bg-dark-2 dark:text-gray-300">
                                    ...
                                    {["C","D","E","F","G","H","I","J","K","L","M","N"].map((letter) => (
                ...
                <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark">
                    <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-lg font-bold text-dark dark:text-white">
                            Raw JSON Payload (
                            {reportData?.ReturnItemsList?.length || 1008} items)
                        </h2>
                        <button
                            onClick={() => {
