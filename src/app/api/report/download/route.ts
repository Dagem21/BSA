...
export async function GET(request: NextRequest) {
    try {
        ...
        if (fs.existsSync(jsonFilePath)) {
            ...
            const excelBuffer = await generateExcelFromJson(
                jsonContent,
                excelFilePath
            );
        ...
        return NextResponse.json(
            { error: `File "${safeFileName}" not found.` },
            { status: 404 }
        );
    } catch (err: any) {
        ...
        return NextResponse.json(
            { error: err.message || "Failed to download Excel file" },
            { status: 500 }
        );
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!body || typeof body !== "object") {
            return NextResponse.json(
                { error: "Invalid JSON report payload" },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            {
                error:
                    err.message || "Failed to generate Excel from JSON payload"
            },
            { status: 500 }
        );
    }
