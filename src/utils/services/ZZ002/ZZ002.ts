...
// Generate all 1,530 ReturnItems definitions for ZZ002
// 85 rows x 18 columns (Cols C=3 to T=20)
// Codes range from ZZ002_57782 to ZZ002_59311
...
function buildZZ002Definitions() {
    const defs = [];
    let codeIndex = 57782;
...
const ZZ002_ITEM_DEFINITIONS = buildZZ002Definitions();

export function processZZ002(worksheet, instCode, startDate, endDate) {
    ...
    for (let r = startRow; r < startRow + 85; r++) {
        ...
        for (let c = 3; c <= 20; c++) {
            if (itemIndex < ZZ002_ITEM_DEFINITIONS.length) {
                const code = ZZ002_ITEM_DEFINITIONS[itemIndex].code;
    ...
    const returnItemsList = ZZ002_ITEM_DEFINITIONS.map(def => ({
        Code: def.code,
...
export async function jsonToExcelZZ002(jsonPayload) {
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet("IFB Loans Sector & Region");

    // Row 1 Unit Banner Right-Aligned in Col R
    sheet.getCell("R1").value = "In Millions of Birr";
    sheet.getCell("R1").font = { name: "Arial", size: 10, bold: true };
    sheet.getCell("R1").alignment = { horizontal: "right" };
    ...
    const sectorBanners = [
        ...
        { colStart: 9, colEnd: 11, label: "Regional Gov't" },
    ...
    sectorBanners.forEach(sec => {
        sheet.mergeCells(2, sec.colStart, 2, sec.colEnd);
        const cell = sheet.getCell(2, sec.colStart);
    ...
    for (let i = 0; i < 6; i++) {
        sheet.getCell(3, 3 + i * 3).value = subHeaders[0];
        sheet.getCell(3, 4 + i * 3).value = subHeaders[1];
        sheet.getCell(3, 5 + i * 3).value = subHeaders[2];
    }
    sheet.getRow(3).font = { name: "Arial", size: 10, bold: true };
    ...
    let itemIndex = 0;
    let currentRow = 4; // Data starts directly at Row 4!

    // Track vertical column totals for Row 88 (Total Amount)
    ...
    REGION_NAMES.forEach((region, rIdx) => {
        ...
        subLabels.forEach((sub, sIdx) => {
            ...
            const rowVals = [];
            for (let c = 3; c <= 20; c++) {
                if (itemIndex < ZZ002_ITEM_DEFINITIONS.length - 18) {
                    const code = ZZ002_ITEM_DEFINITIONS[itemIndex].code;
...
export async function processZZ002Report(
    ...
    try {
        ...
        const rawJson = processZZ002(worksheet, instCode, startDateStr, endDateStr);
        ...
        if (outputExcelPath) {
            ...
            const outWorkbook = await jsonToExcelZZ002(jsonPayload);
        ...
        console.error("Error processing ZZ002 report:", err);
        return {
            success: false,
            error: err.message || "Failed to process ZZ002 report."
        };
    }
}
