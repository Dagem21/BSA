...
export async function processRA002Report(
    instCode: string,
    ...
    try {
        ...
        const worksheet = workbook.getWorksheet("Loan by range and region");
        if (!worksheet) {
            throw new Error(
                "Sheet 'Loan by range and region' not found in the template."
            );
        }

        const templateDir = path.join(process.cwd(), "templates", "json");
        const filePath = path.join(templateDir, "./RA002.json");
...
async function toExcel(jsonFileName: string, outputExcelPath: string) {
    try {
        ...
        const filePath = path.join(templateDir, "RA002.xlsx");
        const workbook = new ExcelJS.Workbook();
