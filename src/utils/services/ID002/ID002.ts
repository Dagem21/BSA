...
import { ID002Format, getID002SubRows } from "./jsonFormat";
...
export function processID002(worksheet: ExcelJS.Worksheet, options?: { instCode?: string; startDate?: string; endDate?: string }) {
    ...
    // Auto-calculate empty total columns (Cols 9, 10, 11) from <=100k, >100k-1M, >1M
    for (let r = 0; r < grid.length; r++) {
        // Amount Total (col 9) = col 0 + col 3 + col 6
        if (!grid[r][9] || grid[r][9] === "0") {
            const sumAmt = parseNum(grid[r][0]) + parseNum(grid[r][3]) + parseNum(grid[r][6]);
            if (sumAmt !== 0) grid[r][9] = String(sumAmt);
        }
        // Depositors Total (col 10) = col 1 + col 4 + col 7
        if (!grid[r][10] || grid[r][10] === "0") {
            const sumDep = parseNum(grid[r][1]) + parseNum(grid[r][4]) + parseNum(grid[r][7]);
            if (sumDep !== 0) grid[r][10] = String(sumDep);
        }
        // Accounts Total (col 11) = col 2 + col 5 + col 8
        if (!grid[r][11] || grid[r][11] === "0") {
            const sumAcc = parseNum(grid[r][2]) + parseNum(grid[r][5]) + parseNum(grid[r][8]);
            if (sumAcc !== 0) grid[r][11] = String(sumAcc);
        }
    }
