...
// Generate all 1,020 ReturnItems definitions for CM002
// 85 rows x 12 columns (Cols C=3 to N=14)
// Codes range from CM002_46232 to CM002_47251
...
function buildCM002Definitions() {
    const defs = [];
    let codeIndex = 46232;
...
const CM002_ITEM_DEFINITIONS = buildCM002Definitions();

export function processCM002(worksheet, instCode, startDate, endDate) {
    ...
    // Read 85 data rows starting from startRow for CM002 (14 regions x 6 rows + Total)
    for (let r = startRow; r < startRow + 85; r++) {
        const row = worksheet.getRow(r);
        for (let c = 3; c <= 14; c++) {
            if (itemIndex < CM002_ITEM_DEFINITIONS.length) {
                const code = CM002_ITEM_DEFINITIONS[itemIndex].code;
    ...
    const returnItemsList = CM002_ITEM_DEFINITIONS.map(def => ({
        Code: def.code,
    ...
    return {
        ReturnKey: "CDby Range and RegCM002",
...
export async function jsonToExcelCM002(jsonPayload) {
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet("Deposits Range & Region");

    // Row 1: Amount banner right-aligned
    sheet.getCell("L1").value = "In Millions of Birr";
    ...
    REGION_NAMES.forEach((region, rIdx) => {
        const subLabels = [
            ...
            { code: `${rIdx + 1}.3`, label: "Time", isHeader: false },
        ...
        subLabels.forEach((sub, sIdx) => {
            ...
            const rowVals = [];
            for (let c = 3; c <= 14; c++) {
                if (itemIndex < CM002_ITEM_DEFINITIONS.length - 12) {
                    const code = CM002_ITEM_DEFINITIONS[itemIndex].code;
...
export async function processCM002Report(
    ...
    try {
        ...
        const rawJson = processCM002(worksheet, instCode, startDateStr, endDateStr);
        ...
        if (outputExcelPath) {
            ...
            const outWorkbook = await jsonToExcelCM002(jsonPayload);
        ...
        console.error("Error processing CM002 report:", err);
        return {
            success: false,
            error: err.message || "Failed to process CM002 report."
        };
    }
}
