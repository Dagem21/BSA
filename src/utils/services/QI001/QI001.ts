...
import { QI001_DESCRIPTIONS, QI001JsonData } from "./jsonFormat";
...
export function processQI001(
    ...
    const jsonTemplatePath = path.join(templateDir, "QI001.json");
...
export async function jsonToExcelQI001(
    jsonPayload: any,
    outputExcelPath?: string
): Promise<Buffer> {
    const templatePath = path.join(process.cwd(), "templates", "QI001.xlsx");
    ...
    let worksheet =
        workbook.getWorksheet("CAP_ADQ_ITEM_QI001") ||
        workbook.getWorksheet("Sheet1") ||
        workbook.worksheets[0];

    if (!worksheet) {
        worksheet = workbook.addWorksheet("CAP_ADQ_ITEM_QI001");
    }
    ...
    QI001_DESCRIPTIONS.forEach((itemDef) => {
...
export async function processQI001Report(
    instCode: string,
    ...
    try {
        ...
        const templateDir = path.join(process.cwd(), "templates", "json");
        const filePath = path.join(templateDir, "QI001.json");
        const data = await readFile(filePath, "utf-8");
        const jsonTemplate = JSON.parse(data);

        const extractedData = processQI001(worksheet, {
            instCode,
        ...
        await jsonToExcelQI001(mergedJson, outputExcelPath);
        ...
        console.error("Error processing QI001 report:", error);
        return {
            success: false,
            error: error.message || "Failed to process QI001 report."
        };
    }
}
