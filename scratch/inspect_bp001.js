...
async function test() {
    const filePath = path.join(__dirname, '../templates/BP001.xlsx');
    let out = [];
    ...
    fs.writeFileSync(path.join(__dirname, 'bp001_info.txt'), out.join('\n'));
}
test().catch(err => {
    fs.writeFileSync(path.join(__dirname, 'bp001_info.txt'), err.stack || String(err));
});
